...
namespace ZeroHunger.Controllers
{
    public class HomeController : Controller
    {
        ...
        public ActionResult Distributed_Foods()
        {
            ...
            var collectedItems = db.Distributions.Where(d => d.DistributedTime != null).ToList();
            var config1 = new MapperConfiguration(cfg => {
        ...
        {
            var db = new ZeroHungerEntities();
            var employeeIds = db.Users
                .Where(u => u.Type == "Employee")
                .ToList();

            ViewBag.EmployeeIds = employeeIds;
            var collectedItems = db.Distributions.Where(d => d.DistributedLocation == null && d.EmployeeId==null).ToList();
            var config1 = new MapperConfiguration(cfg => {
                cfg.CreateMap<Distribution, DistributionDTO>();
            });
            var mapper1 = new Mapper(config1);
        ...
        [HttpPost]
        public ActionResult Distribution(DistributionDTO distribution)
        {
            ...
            using (var db = new ZeroHungerEntities())
            {
                ...
                {
                    existingEntity.DistributedLocation = distribution.DistributedLocation;
                    existingEntity.EmployeeId =distribution.EmployeeId;
                    db.SaveChanges();
                }
            }
